...
if uploaded_file3 is not None:
    ...
    def DISPATCH(df):
        ...
        df_copy.loc[df_copy['aff_equal'] == True, 'Pax CNT TOT'] = df_copy['PAX TOT'] * (df_copy['Taux de correspondance'])

        df_copy.loc[df_copy['aff_equal'] == False, 'Libellé terminal'] = df_copy['Affectation 1']
        df_copy.loc[df_copy['aff_equal'] == False, 'Pax LOC TOT'] = df_copy['PAX TOT'] * (1 - (df_copy['Taux de correspondance'])) * df_copy['Taux affectation 1']
        df_copy.loc[df_copy['aff_equal'] == False, 'Pax CNT TOT'] = df_copy['PAX TOT'] * (df_copy['Taux de correspondance']) * df_copy['Taux affectation 1']
        df_copy.loc[df_copy['aff_equal'] == False, 'PAX TOT'] = df_copy['PAX TOT'] * df_copy['Taux affectation 1']
        
        df_copy_aff_2 = df_copy[df_copy['aff_equal'] == False].copy()
        
        df_copy_aff_2['Libellé terminal'] =  df_copy_aff_2['Affectation 2']
        df_copy_aff_2.loc[df_copy_aff_2['aff_equal'] == False, 'Pax LOC TOT'] = df_copy_aff_2['PAX TOT'] * (1 - (df_copy_aff_2['Taux de correspondance'])) * (1 - (df_copy_aff_2['Taux affectation 1']))
        df_copy_aff_2.loc[df_copy_aff_2['aff_equal'] == False, 'Pax CNT TOT'] = df_copy_aff_2['PAX TOT'] * (df_copy_aff_2['Taux de correspondance']) * (1 - (df_copy_aff_2['Taux affectation 1']))
        df_copy_aff_2.loc[df_copy_aff_2['aff_equal'] == False, 'PAX TOT'] = df_copy_aff_2['PAX TOT'] * (1 - (df_copy_aff_2['Taux affectation 1']))
        

            # Vérifier si affectation 1 est bien dans les libellés terminaux (EK, EL, EM)
        #print('Erreur dans Affectation OAL : affectation 1 ou 2 invalides !')
        
        df_oal_concat = pd.concat([df_copy, df_copy_aff_2])   
        df_oal_concat.reset_index(inplace=True)
        del df_oal_concat['index']
        del df_oal_concat['Compagnie']
        del df_oal_concat['Code IATA']
        del df_oal_concat['Affectation 1']
        del df_oal_concat['Affectation 2']
        del df_oal_concat['Taux affectation 1']
        del df_oal_concat['Taux de correspondance']
        del df_oal_concat['aff_equal']
