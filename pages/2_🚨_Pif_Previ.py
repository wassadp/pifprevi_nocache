...
if uploaded_file is not None:
    ...
    ### DISPATCH ###       
        ...
        # st.write(dispatch['L CTR'].sum() + dispatch['K CTR'].sum() + dispatch['M CTR'].sum() + 
        # dispatch['Galerie EF'].sum() + dispatch['L CNT'].sum())
        # st.write('L CNT')
        # st.write(dispatch['L CNT'].sum())
        ...
        l_courbe_geo_t.append((liste_df_courbe_presentation_terminal[0][0], courbe(dec + 9, liste_df_courbe_presentation_terminal[0][1]))) #AC
        l_courbe_geo_t.append((liste_df_courbe_presentation_terminal[1][0], courbe(dec + 10, liste_df_courbe_presentation_terminal[1][1]))) #BD
        l_courbe_geo_t.append((liste_df_courbe_presentation_terminal[2][0], courbe(dec + 3, liste_df_courbe_presentation_terminal[2][1]))) #2E
        l_courbe_geo_t.append((liste_df_courbe_presentation_terminal[3][0], courbe(dec + 2, liste_df_courbe_presentation_terminal[3][1]))) #2F
        ...
        l_courbe_geo_t.append((liste_df_courbe_presentation_terminal[5][0], courbe(dec + 6, liste_df_courbe_presentation_terminal[5][1]))) #T3
        l_courbe_geo_t.append((liste_df_courbe_presentation_terminal[6][0], courbe(dec + 12, liste_df_courbe_presentation_terminal[6][1]))) #T1 Inter
        l_courbe_geo_t.append((liste_df_courbe_presentation_terminal[7][0], courbe(dec + 6, liste_df_courbe_presentation_terminal[7][1]))) #T1_5
        l_courbe_geo_t.append((liste_df_courbe_presentation_terminal[8][0], courbe(dec + 6, liste_df_courbe_presentation_terminal[8][1]))) #T1_6
    ...
    #                    M CTR
        ...
        #        Terminal 1 Schengen 5 et 6
            ...
            st.write(df_site['L CTR']['charge'].sum())
        ...
        #permet de lisser et decaler les courbes des pif suivant 
        # (window > choisie arbitrairement (visuellement))
        # (shift > choisie arbitrairement (visuellement & réalisé))
        pif_to_roll = ['K CNT', 'L CNT', 'L CTR', 'M CTR', 'Galerie EF']
        
        for pif in pif_to_roll:
            x['charge'][x['site']==pif] = x[x['site']==pif]['charge'].rolling(window=3).mean()
            x['charge'][x['site']==pif] = x[x['site']==pif]['charge'].shift(1)
            x['charge'][x['site']==pif] = x[x['site']==pif]['charge'].fillna(0)
        
        x['charge'].fillna(0, inplace=True)
